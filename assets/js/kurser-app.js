(() => {
 const elResultCount = document.getElementById("resultCount");  const data = Array.isArray(window.TFF_KURSER) ? window.TFF_KURSER : [];
  ...
  const applyFilters = (items) => {
    ...
    return items.filter(item => {
      ...
      const textMatch = q.length === 0 ? true : (title.includes(q) || purpose.includes(q) || modulesText.includes(q));
    ...
    items.forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div class="course-card__body">
          ...
          <p class="course-card__purpose">${escapeHtml(purpose)}</p>
  ...
  elReset.addEventListener("click", () => {
    ...
    elCategoryList.querySelectorAll('input[type="checkbox"][data-cat]').forEach(cb => cb.checked = false);
